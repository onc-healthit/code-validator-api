...
public class ClassCodeValidator extends NodeValidator {
	...
	public List<VocabularyValidationResult> validateNode(ConfiguredValidator configuredValidator, XPath xpath,
		...
		String classCode;
		try {
			classCode = vn.getAttrVal("classCode")!=-1 ? vn.toString(vn.getAttrVal("classCode")).toUpperCase() : "";
		} catch (Exception e) {
			throw new RuntimeException("ERROR getting node values " + e.getMessage());
		}

		List<String> allowedConfiguredCodeSystemOids = new ArrayList<>(
				Arrays.asList(configuredValidator.getAllowedValuesetOids().split(",")));
